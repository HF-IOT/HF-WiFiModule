 ...
 * Returns      : result
*******************************************************************************/
LOCAL int ICACHE_FLASH_ATTR device_get(struct jsontree_context *js_ctx)
{
    const char *path = jsontree_path_name(js_ctx, js_ctx->depth - 1);

    if (os_strncmp(path, "manufacture", 11) == 0) {
        jsontree_write_string(js_ctx, "HuaFan IOT team");
    } else if (os_strncmp(path, "product", 7) == 0) {
        jsontree_write_string(js_ctx, "WiFiModule");
    }
    return 0;
}
 ...
 * Returns      : result
*******************************************************************************/
LOCAL int ICACHE_FLASH_ATTR userbin_get(struct jsontree_context *js_ctx)
{
    const char *path = jsontree_path_name(js_ctx, js_ctx->depth - 1);
    char string[32];
...
LOCAL int ICACHE_FLASH_ATTR version_get(struct jsontree_context *js_ctx)
{
    ...
    if (os_strncmp(path, "hardware", 8) == 0) {
    	...
    	os_sprintf(string,"%s",HFWIFIMODE);
    }
    jsontree_write_string(js_ctx, string);
    return 0;
}
...
LOCAL int ICACHE_FLASH_ATTR connect_status_get(struct jsontree_context *js_ctx)
{
    const char *path = jsontree_path_name(js_ctx, js_ctx->depth - 1);
    if (os_strncmp(path, "status", 8) == 0) {
 ...
 * Returns      : result
*******************************************************************************/
LOCAL int ICACHE_FLASH_ATTR status_get(struct jsontree_context * js_ctx)
{
	// change
	os_printf("status_get\r\n");
	jsontree_write_int(js_ctx,0);
    return 0;
}

LOCAL int ICACHE_FLASH_ATTR mytest_get(struct jsontree_context * js_ctx)
{
	// change
	os_printf("mytest_get\r\n");
	jsontree_write_int(js_ctx,0);
    return 0;
}
 ...
 * Returns      : result
*******************************************************************************/
LOCAL int ICACHE_FLASH_ATTR status_set(struct jsontree_context *js_ctx, struct jsonparse_state *parser)
{
    int type;
	os_printf("status_set\r\n");
...
LOCAL struct jsontree_callback mytest_callback =
    JSONTREE_CALLBACK(mytest_get, NULL);
...
JSONTREE_OBJECT(myjson_tree,
                JSONTREE_PAIR("status", &mytest_callback ));
JSONTREE_OBJECT(mytest_tree,
                JSONTREE_PAIR("MYTEST", &myjson_tree));
#endif
 ...
 * Returns      : result
*******************************************************************************/
LOCAL int ICACHE_FLASH_ATTR wifi_station_get(struct jsontree_context *js_ctx)
{
    const char *path = jsontree_path_name(js_ctx, js_ctx->depth - 1);
 ...
 * Returns      : result
*******************************************************************************/
LOCAL int ICACHE_FLASH_ATTR wifi_station_set(struct jsontree_context *js_ctx, struct jsonparse_state *parser)
{
    int type;
    uint8 station_tree;
 ...
 * Returns      : result
*******************************************************************************/
LOCAL int ICACHE_FLASH_ATTR wifi_softap_get(struct jsontree_context *js_ctx)
{
    const char *path = jsontree_path_name(js_ctx, js_ctx->depth - 1);
    struct ip_info ipconfig;
    uint8 buf[20];
 ...
 * Returns      : result
*******************************************************************************/
LOCAL int ICACHE_FLASH_ATTR wifi_softap_set(struct jsontree_context *js_ctx, struct jsonparse_state *parser)
{
    int type;
    uint8 softap_tree;
 ...
 * Returns      : result
*******************************************************************************/
LOCAL int ICACHE_FLASH_ATTR scan_get(struct jsontree_context *js_ctx)
{
    const char *path = jsontree_path_name(js_ctx, js_ctx->depth - 1);
...
LOCAL void ICACHE_FLASH_ATTR json_send(void *arg, ParmType ParmType)
{
    ...
    switch (ParmType) {
        ...
        case USER_BIN:
        	...
        	break;
		case MYTEST:
			json_ws_send((struct jsontree_value *)&mytest_tree, "MYTEST", pbuf);
			break;
        case SCAN: {
            u8 i = 0;
            u8 scancount = 0;
 ...
 * Returns      : none
*******************************************************************************/
LOCAL void ICACHE_FLASH_ATTR local_upgrade_download(void * arg,char *pusrdata, unsigned short length)
{
    char *ptr = NULL;
    char *ptmp2 = NULL;
...
#endif
					else if (os_strcmp(pURL_Frame->pFilename, "MYTEST") == 0){
						json_send(ptrespconn, MYTEST);
					}
